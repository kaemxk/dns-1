...
describe('ContentSlider', () => {
  it('обработки ошибки загрузки данных', () => {
    vi.spyOn(baseApi, 'useGetSliderContentQuery').mockReturnValue({
      data: null,
      isLoading: false,
      error: true,
      refetch: vi.fn(),
  ...
  it('рендер данных', () => {
    ...
    renderContentSlider()
    expect(screen.getByText(/Старт продаж!/i)).toBeInTheDocument()
  })

  it('рендер данных + фильтрация', () => {
    vi.spyOn(baseApi, 'useGetSliderContentQuery').mockReturnValue({
      data: mockData,
      isLoading: false,
      error: false,
