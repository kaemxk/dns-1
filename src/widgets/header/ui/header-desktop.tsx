...
import {
    DNSCityData,
    DNSSupportData,
    useLazyGetCityListQuery, useLazyGetMenuHeaderQuery
...
export const HeaderDesktop = () => {
    ...
    const [cityId, setCityId] = useState(localStorage.getItem("city_id") || "30b7c1f3-03fb-11dc-95ee-00151716f9f5")
    const [getCitiesTrigger, getCitiesResult, ]: [() => void, { data?: DNSCityData[] | undefined }, {lastArg: void} ] = useLazyGetCityListQuery()
    const [getMenuHeaderTrigger, getMenuHeaderResult, ]: [(header: string) => void, { data?: DNSSupportData | undefined }, {lastArg: string} ] = useLazyGetMenuHeaderQuery()
    ...
    return (
        <div className='header-desktop hidden lg:block'>
            <div className='header-top w-full border-none bg-white leading-[21px] text-[#4e4e4e] xl:pl-2.5 xl:pr-2.5'>
                <div
                    className='header-top__container mx-auto my-0 box-content flex min-h-[75%] max-w-[1380px] flex-nowrap justify-between pb-2 pl-0 pt-5 lg:w-[960px] xl:w-full'>
                        ...
                        <ul className='header-top-menu__common-list mb-0 ml-0 mr-auto mt-0 flex grow whitespace-nowrap p-0'>
                            ...
                            <li className='mr-5 lg:mr-3'>
                                <Link to='#' className='header-top-menu__common-link hover:text-orange-500'>
                                    Акции
                                    <TbPointFilled
                                        className='header-top-menu__dot-svg ml-1 mr-1 inline-block text-orange-500'/>
                                </Link>
                            </li>
            ...
            <nav className={classesScrollHeader}>
                <div
                    className='header-bottom__container mx-auto my-0 box-content flex w-full max-w-[1380px] items-center justify-start lg:w-[960px] xl:w-full'>
                    <div
                        ...
                        >
                            <div
                                className='header-bottom__logo relative inline-block w-[67px] align-middle transition-[.3s] xl:w-[80px]'>
